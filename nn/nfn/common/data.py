...
def params_to_state_dicts(keys, wsfeat: WeightSpaceFeatures) -> List[OrderedDict]:
    ...
    while layer_idx < len(keys):
        for batch_idx in range(batch_size):
            state_dicts[batch_idx][keys[layer_idx]] = wsfeat.weights[layer_idx // 2][
                batch_idx
            ].squeeze(0)
        layer_idx += 1
        for batch_idx in range(batch_size):
            state_dicts[batch_idx][keys[layer_idx]] = wsfeat.biases[layer_idx // 2][
                batch_idx
            ].squeeze(0)
        layer_idx += 1
    return state_dicts
...
def network_spec_from_wsfeat(
    wsfeat: WeightSpaceFeatures, set_all_dims=False
) -> NetworkSpec:
