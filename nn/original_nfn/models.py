...
from nn.original_nfn.layers import (
    ChannelDropout,
...
class NormalizingModule(nn.Module):
    ...
    def set_stats(self, mean_std_stats):
        if self.normalize:
            ...
            for i, (w, b) in enumerate(zip(weight_stats, bias_stats)):
                ...
                std_weights = torch.where(
                    std_weights < 1e-5, torch.ones_like(std_weights), std_weights
                )
                std_bias = torch.where(
                    std_bias < 1e-5, torch.ones_like(std_bias), std_bias
                )
    ...
    def _normalize(self, params):
        ...
        for i, (w, b) in enumerate(params):
            mean_weights_i, std_weights_i = getattr(self, f"mean_weights_{i}"), getattr(
                self, f"std_weights_{i}"
            )
            mean_bias_i, std_bias_i = getattr(self, f"mean_bias_{i}"), getattr(
                self, f"std_bias_{i}"
            )
...
class InvariantNFN(NormalizingModule):
    ...
    def __init__(
        ...
        for num_channels in hchannels:
            layers.append(
                MODE2LAYER[mode](
                    network_spec, in_channels=prev_channels, out_channels=num_channels
                )
            )
