...
from nn.original_nfn.common import NetworkSpec, WeightSpaceFeatures
from nn.original_nfn.layers.layer_utils import (
    set_init_,
...
class HNPLinear(nn.Module):
    ...
    def forward(self, wsfeat: WeightSpaceFeatures):
        ...
        cmL = torch.flatten(
            col_means[-1], start_dim=-2
        )  # b c_in nrowL -> b (c_in nrowL)
        final_bias = torch.flatten(
            biases[-1], start_dim=-2
        )  # b c_in nrow -> b (c_in nrow)
...
class NPAttention(nn.Module):
    def __init__(
        ...
        self.split_heads = Rearrange(
            "b (nh ch) ... -> b nh ch ...", nh=self.nh, ch=3 * self.ch
        )
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> WeightSpaceFeatures:
        ...
        qkv_avgs = torch.stack(
            rowcol_means + bias_means, dim=-2
        )  # (B, nh, 2 * n_layers, 3ch)
        ...
        for i in range(-1, len(wsfeat)):
            ...
            if i > -1:
                ...
                out_weights_i = torch.flatten(
                    out[:, :, :n_im1].permute(0, 1, 3, 4, 2), start_dim=1, end_dim=2
                )
                if not self.share_projections:
                    out_weights_i = self.unproject_weight[i](out_weights_i)
                out_weights[i] += out_weights_i
                # b nh 1 ch n_i -> b (nh ch) n_i
                out_biases[i] += torch.flatten(
                    out[:, :, n_im1 : n_im1 + 1].squeeze(2), start_dim=1, end_dim=2
                )
                idx = n_im1 + 1
            if i < len(wsfeat) - 1:
                # b nh n_ip1 ch n_i -> b (nh ch) n_ip1 n_i
                out_weights_ip1 = torch.flatten(
                    out[:, :, idx:].transpose(2, 3), start_dim=1, end_dim=2
                )
                if not self.share_projections:
                    out_weights_ip1 = self.unproject_weight[i + 1](out_weights_ip1)
