...
class IOSinusoidalEncoding(nn.Module):
    def __init__(
        self, network_spec: NetworkSpec, max_freq=10, num_bands=6, enc_layers=True
    ):
        super().__init__()
        self.network_spec = network_spec
...
class LearnedPosEmbedding(nn.Module):
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> WeightSpaceFeatures:
        ...
        for i in range(len(self.network_spec)):
            ...
            weight = (
                weight
                + self.weight_emb.weight[i][(None, Ellipsis, None, None, *filter_dims)]
            )
            bias = bias + self.bias_emb.weight[i][None, :, None]
            if i == 0:
                weight = (
                    weight
                    + self.inp_weight_arrange(self.inp_emb.weight)[
                        (Ellipsis, *filter_dims)
                    ]
                )
            if i == len(wsfeat.weights) - 1:
                weight = (
                    weight
                    + self.out_weight_arrange(self.out_emb.weight)[
                        (Ellipsis, *filter_dims)
                    ]
                )
